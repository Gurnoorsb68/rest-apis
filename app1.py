...
def create_item(name):
    ...
    for x in stores:
        if x["name"] == name:
            ...
            return  new_itm, 201
        return  {"message" : "missing store"} , 404 #error displays

@app1.get("/store/<string:name>")   #API to get perticular store
...
@app1.get("/store/<string:name>/itemdetails")   #API to get Items of particular store
def get_item(name):
    for x in stores:
        if x["name"] == name:
            return  {"items" : x["items"]}  # returning dictionary is better, u could have also done x["items"] so
